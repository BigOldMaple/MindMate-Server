...
router.post('/generate-test-data', authenticateToken, async (req: any, res) => {
  try {
    ...
    if (!pattern || !startDate || !days) {
      return res.status(400).json({ error: 'Pattern, start date, and days are required' });
    }

    if (!['good', 'declining', 'critical', 'improving', 'fluctuating'].includes(pattern)) {
    ...
    // Call the test data generator service
    const result = await healthDataSyncService.generateTestData(
      req.userId,
      pattern,
      parsedStartDate,
      days
...
// Clear test data for a specified date range
router.post('/clear-test-data', authenticateToken, async (req: any, res) => {
  try {
    ...
    // Call service to delete test data
    const result = await healthDataSyncService.clearTestData(
      req.userId,
      parsedStartDate,
      parsedEndDate
    );
